...
const stats = [
  { value: '1', label: 'national hub', target: 'about' },
  { value: '4', label: 'exemplar programmes', target: 'platform' },
  { value: '6', label: 'model families', target: 'models' },
  { value: '2', label: 'user routes', target: 'access' },
];
...
export default function OrbDigitalPlatformMockup() {
  ...
  const filteredModels = useMemo(() => {
    return models.filter((m) => {
      const matchQuery =
        query.trim().length === 0 ||
        [m.name, m.type, m.summary, ...m.tags].join(' ').toLowerCase().includes(query.toLowerCase());
          ...
          <Card className="rounded-3xl border-slate-200 shadow-sm">
            <CardContent className="p-6">
              ...
              <div className="mt-4 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <div className="mt-2 rounded-2xl bg-slate-900 p-4 text-sm leading-6 text-white">
